...
public class VideoPlaybackController : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (Input.touchCount > 0)
		{
			...
			{
				if (Vector2.Distance(mTouchStartPos, touch.position) > 40)	// (2)
				{
					// Touch moved too far
					mTouchMoved = true;
				}
			...
			{
				if (!mTouchMoved && mTimeElapsed < 1.0)	// (3) 시작
				{
					if (mTapped)
					{
						// Second tap
		...
		if (mTapped)
		{
			if (mTimeElapsedSinceTap >= 0.5f)	// (4)
			{
				// Not a double tap
				HandleTap();
				mTapped = false;
	...
	/// </summary>
	private void HandleDoubleTap()	// (6)
	{
		// Find out which video was tapped, if any
		VideoPlaybackBehaviour video = PickVideo(mTouchStartPos);
	...
	/// </summary>
	private VideoPlaybackBehaviour PickVideo(Vector3 screenPoint)	// (7)
	{
