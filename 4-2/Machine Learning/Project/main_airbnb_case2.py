...
def start(df_train):
    ...
    df_train, _ = train_test_split(df_train, test_size=0.98, random_state=42, stratify=df_train["age_temp"])
    ...
    for col in ["gender", "signup_method", "language", "affiliate_channel", "affiliate_provider", "first_affiliate_tracked", "signup_app", "first_device_type", "first_browser",  "action", "action_type", "action_detail" ,"device_type"] :
        print("feature : " + col)
    ...
    num_pipeline = Pipeline([
        ("select_numeric", DataFrameSelector(["age", "signup_flow", "year", "month", "secs_elapsed"])),
    ])

    cat_pipeline = Pipeline([
        ("select_cat", DataFrameSelector(["gender", "signup_method", "language", "affiliate_channel", "affiliate_provider", "first_affiliate_tracked", "signup_app", "first_device_type", "first_browser", "action", "action_type", "action_detail" ,"device_type"])),
...
def check_dataSet(df_train):
    ...
    df_train.loc[df_train['age'] < 5, 'age'] = 1
    df_train.loc[df_train['age'] > 100, 'age'] = 99
    df_train['age'] = df_train['age'].fillna(float(int(df_train['age'].median()))) # 나이를 증간값으로 줬을때
...
if __name__ == "__main__":
    ...
    df_train_user = load_data("train_users_2.csv")
    df_train_sess = load_data("sessions.csv")

    print(df_train_sess.info())
    print(df_train_sess.isnull().sum()) # 빠진값 확인

    df_train_sess = df_train_sess.dropna(axis=0)

    print("NaN 제거 이후")
    print(df_train_sess.isnull().sum()) # 빠진값 확인
    print(df_train_sess.describe())

    df_train_sess["id"] = df_train_sess["user_id"].copy()
    df_train_sess = df_train_sess.drop("user_id", axis=1)
    df_train = pd.merge(df_train_user, df_train_sess, on="id")
