...
class Date {
//	클래스를 friend로 설정하면 friend로 설정된 클래스의 모든 멤버 함수가 현재 클래스의
//	private 멤버 변수에 자유롭게 접근할 수 있다.
//	완전히 구현된 클래스만 friend 클래스로 지정할 수 있음에 주의하자
//	Time 클래스의 모든 멤버 함수(today(Date *d), today(Date &d))가 Date 클래스의 
...
//	Time 클래스의 멤버 함수이므로 Time 클래스의 private 멤버 변수에 접근할 수 있다.
	void today(Date *d) {
